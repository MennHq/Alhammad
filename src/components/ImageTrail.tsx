...
class ImageItem {
  ...
  getRect() {
    ...
    if (!this.rect || this.rect.height === 0) {
      const img = this.DOM.el.querySelector('img');
      if (img && img.naturalHeight) {
        const ratio = img.naturalHeight / img.naturalWidth;
        const width = this.rect?.width || 260;
        this.rect = {
          x: this.rect?.x || 0,
          y: this.rect?.y || 0,
          left: this.rect?.left || 0,
          top: this.rect?.top || 0,
          right: (this.rect?.left || 0) + width,
          bottom: (this.rect?.top || 0) + (width * ratio),
          width: width,
          height: width * ratio
        } as DOMRect;
      } else {
...
class ImageTrailVariant1 {
  ...
  constructor(container: HTMLElement) {
    ...
    container.addEventListener('touchmove', this.handlePointerMove as any);
    ...
    this.initRender = ev => {
      ...
      container.removeEventListener('touchmove', this.initRender as any);
    };
    container.addEventListener('mousemove', this.initRender);
    container.addEventListener('touchmove', this.initRender as any);
  }
  ...
  destroy() {
    ...
    this.container.removeEventListener('touchmove', this.handlePointerMove as any);
    this.container.removeEventListener('mousemove', this.initRender);
    this.container.removeEventListener('touchmove', this.initRender as any);
...
class ImageTrailVariant6 {
  ...
  showNextImage() {
    ...
    let grayscaleValue = this.mapSpeedToGrayscale(speed, 90, 0);
...
export default function ImageTrail({ items = [], variant = 1, parentRef }: ImageTrailProps) {
  const containerRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    if (typeof window !== 'undefined' && window.innerWidth < 768) {
      return; // Do not initialize image trail animation on mobile viewports
    }
    ...
    const instance = new Cls(targetElement);

    return () => {
      instance.destroy();
    };
  }, [variant, items, parentRef]);

  return (
    <div className="content-trail hidden md:block" ref={containerRef}>
      {items.map((url, i) => (
